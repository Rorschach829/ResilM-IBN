...
# ax.set_title("实验2：不同意图的修复动作触发成功率", fontsize=14, fontproperties=prop)
ax.set_xticks(x_pos)
...
# ======== 9. 图例（用不同填充线区分） ========
legend_elements = [
    Patch(facecolor='white', edgecolor='black', hatch='xx', label='应触发修复'),
    Patch(facecolor='white', edgecolor='black', hatch='//', label='无需修复')
]
ax.legend(
...
output_path = os.path.join(latest_dir, "exp2_repair_triggered_rate_bw.png")
